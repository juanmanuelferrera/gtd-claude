...
function addTimeBuffer(time) {
    const [hour, minute] = time.split(':').map(Number);
    const totalMinutes = hour * 60 + minute + 30; // Add 30 minute buffer
    const newHour = Math.floor(totalMinutes / 60);
    const newMinute = totalMinutes % 60;
    return `${String(newHour).padStart(2, '0')}:${String(newMinute).padStart(2, '0')}`;
}

// Toggle time block collapse/expand functionality
function toggleTimeBlock(timeId) {
    console.log('🔄 Toggling time block:', timeId);
    
    const content = document.getElementById(`content-${timeId}`);
    const arrow = document.getElementById(`arrow-${timeId}`);
    
    if (!content || !arrow) {
        console.error('❌ Time block elements not found:', timeId);
        return;
    }
    ...
    const collapseStates = JSON.parse(localStorage.getItem('timeblock_collapse_states') || '{}');
    collapseStates[timeId] = !isCollapsed;
    localStorage.setItem('timeblock_collapse_states', JSON.stringify(collapseStates));
    
    console.log('✅ Time block toggled:', timeId, 'collapsed:', !isCollapsed);
}

// Make mega time blocks functions globally available
window.calculateMegaTimeBlocks = calculateMegaTimeBlocks;
window.toggleTimeBlock = toggleTimeBlock;
