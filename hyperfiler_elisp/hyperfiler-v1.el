  ...
  (let ((storage-file (expand-file-name "hyperfiler-tasks.org" user-emacs-directory))
    ...
    (when (file-exists-p storage-file)
      (condition-case err
        ...
        (error
         (message "Error loading tasks from org: %s. Starting with empty task list." (error-message-string err))
         (setq hyperfiler-tasks nil))))
    ...
    (message "Loaded %d tasks, %d archived, %d lists" (length hyperfiler-tasks) (length hyperfiler-archived-tasks) (length hyperfiler-lists))))
...
(defun hyperfiler--export-today-to-html ()
  ...
  (let* ((today (hyperfiler--get-current-date))
         (today-tasks (hyperfiler--get-tasks-from-org-for-date today))
...
(defun hyperfiler ()
  ...
  (interactive)
  (hyperfiler-load-tasks-from-local-storage)
  (hyperfiler--setup-file-watchers)
  (hyperfiler--start-auto-update-timer)
  ;; Generate initial HTML files
  (hyperfiler--auto-generate-all-html)
  (message "HyperFiler started with automatic HTML generation!")
  (hyperfiler-show-today-view))
