// Offline Sync Queue Module
// Handles offline operations and syncs them when connection is restored
// Maintains "server wins" conflict resolution strategy

// Storage keys
export const OFFLINE_QUEUE_KEY = 'hyperfiler_offline_queue';
export const OFFLINE_STATUS_KEY = 'hyperfiler_offline_status';
 ...
 * @returns {Object} Status object with isOnline, queueLength, etc.
 */
...
export async function deleteTaskFromServer(taskId) {
    try {
        // Access global API_BASE and fetchWithAuth
        const response = await window.fetchWithAuth(`${window.API_BASE}/api/tasks/${taskId}`, {
            method: 'DELETE'
        });
 ...
 * @returns {Promise<boolean>} True if successful
 */
export async function deleteListFromServer(listId) {
    try {
        const response = await window.fetchWithAuth(`${window.API_BASE}/api/lists/${listId}`, {
            method: 'DELETE'
        });
...
export async function deleteTemplateFromServer(templateId) {
    try {
        const response = await window.fetchWithAuth(`${window.API_BASE}/api/templates/${templateId}`, {
            method: 'DELETE'
        });
 ...
 * @returns {Promise<boolean>} True if successful
 */
export async function executeQueuedOperation(operation) {
    try {
        switch (operation.type) {
            case QUEUE_OPERATIONS.CREATE_TASK:
            case QUEUE_OPERATIONS.UPDATE_TASK:
                // For tasks, upload all tasks to let server handle conflicts
                if (typeof window.uploadAllTasks === 'function') {
            ...
            case QUEUE_OPERATIONS.DELETE_TASK:
                // For deletions, call the specific API
            ...
            case QUEUE_OPERATIONS.UPDATE_LIST:
                if (typeof window.uploadAllLists === 'function') {
                    await window.uploadAllLists();
                    return true;
                }
                break;
            ...
            case QUEUE_OPERATIONS.UPDATE_TEMPLATE:
                if (typeof window.uploadAllTemplates === 'function') {
                    await window.uploadAllTemplates();
                    return true;
                }
                break;
 ...
 * @returns {Promise<Object>} Result object with success, processed, failed counts
 */
export async function processOfflineQueue() {
    ...
    if (processed > 0) {
        ...
        try {
            ...
            if (typeof window.comprehensiveSync === 'function') {
                await window.comprehensiveSync();
 ...
 * @returns {boolean} True if operation was queued or will proceed
 */
export function offlineAwareTaskOperation(task, operationType = 'update') {
    if (navigator.onLine) {
 ...
 * @returns {boolean} True if operation was queued or will proceed
 */
export function offlineAwareTaskDeletion(taskId) {
    if (navigator.onLine) {
