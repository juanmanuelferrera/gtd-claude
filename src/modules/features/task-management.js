...
let tasks = []; // Synced with window.tasks
let currentEditTaskId = null;
let undoStack = [];
let maxUndoSteps = 5;
let draggedTask = null;
let isSaving = false;
let eventTaskIds = new Set();
let customTemplates = [];
...
export function saveEventRegistry() {
    try {
...
export function markAsEvent(taskId) {
    eventTaskIds.add(taskId);
    saveEventRegistry();
}

export function unmarkAsEvent(taskId) {
    eventTaskIds.delete(taskId);
    saveEventRegistry();
}

export function isRegisteredEvent(taskId) {
 ...
 */
export function healEventProperties() {
    let healedCount = 0;
    tasks.forEach(task => {
 ...
 */
export function cleanEventRegistry() {
 ...
 */
export function animateTaskCompletion(taskElement) {
 ...
 */
export function quickAddTaskWithTemplate(templateName) {
 ...
 * Update task date
 */
export async function updateTaskDate(taskId, newDate, event) {
 ...
 */
export function openDatePicker(taskId, event) {
    event.stopPropagation();
 ...
 */
export function openTimePicker(taskId, event) {
    event.stopPropagation();
 ...
 * Update task time
 */
export async function updateTaskTime(taskId, newTime, event) {
 ...
 */
export function duplicateTask(taskId, event) {
    event.stopPropagation();
 ...
 */
export function editTask(taskId, event) {
 ...
 * Close task modal
 */
export function closeTaskModal(event) {
 ...
 */
export function saveStateForUndo(action, task = null) {
    const state = {
        action: action,
 ...
 */
export function toggleTaskComplete(taskId, event) {
    if (event) event.stopPropagation();
 ...
 */
export function toggleTaskStatus(taskId, event) {
 ...
 */
export async function delayTask(taskId, days, event) {
    if (event) event.stopPropagation();
...
// Functions are exported as ES6 modules